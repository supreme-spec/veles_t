...
### Серверы Велес Вояж
- **Основной сервер:** [Заполнить при деплое на production]
- **Резервный сервер:** [Заполнить при деплое на production]
- **Dev-среда:** [Заполнить при деплое на production]

### Cloudflare (если используется как reverse proxy)
- `173.245.48.0/20`
- `103.21.244.0/22`
- `103.22.200.0/22`
- `103.31.4.0/22`
...
### Требуется
- [x] Заполнить IP-адреса для белого списка (офисный IP: 222.167.212.52)
- [x] Предоставить доступ к тестовой среде (https://veles-voyage.ru)
- [x] Заполнить пре-сертификационный чеклист (документ обновлён)
- [x] Создать workflow-диаграмму (docs/ostrovok-workflow-diagram.md)
- [x] Собрать логи API-вызовов (logs/ostrovok-test-logs.json)
